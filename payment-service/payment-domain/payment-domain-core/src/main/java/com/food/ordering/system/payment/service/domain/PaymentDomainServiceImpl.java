...
public class PaymentDomainServiceImpl implements PaymentDomainService{
    ...
    public PaymentEvent validateAndCancelPayment(Payment payment,
        ...
        if(failureMessages.isEmpty()){
            ...
            return new PaymentCancelledEvent(payment, ZonedDateTime.now(ZoneId.of(DomainConstants.UTC)), paymentCancelledEventDomainEventPublisher);
        }else{
    ...
    private void validateCreditHistory(CreditEntry creditEntry,
                                       List<CreditHistory> creditHistories,
                                       List<String> failureMessages) {
        Money totalCreditMoney = getTotalHistoryAmount(creditHistories, TransactionType.CREDIT);
        Money totalDebitMoney = getTotalHistoryAmount(creditHistories, TransactionType.DEBIT);
        if(totalDebitMoney.isGreaterThan(totalCreditMoney)){
        ...
        if(!(creditEntry.getTotalCreditAmount().equals
                (totalCreditMoney.subtract(totalDebitMoney)))){
            log.error("Credit history total is not equal to current credit for customer id: {}!",
                    creditEntry.getCustomerId().getValue());
            failureMessages.add("Customer history total is not equal to current credit for customer id: "
                    + creditEntry.getCustomerId().getValue() + "!");
        }
    }
